...
Error Builder::build(StringRef Producer, SmallVector<uint8_t, 0> &ODRTab) {
  storage::Header Hdr;
  ...
  ArrayRef<uint8_t> Input(reinterpret_cast<uint8_t*>(ZODRTab.data()), ZODRTab.size());
  compression::zlib::compress(Input, ODRTab);
